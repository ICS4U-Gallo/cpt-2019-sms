    ...
    def on_draw(self):
        ...
        if end_game_condition_1 or end_game_condition_2:
            ...
            if end_game_condition_2:
                ...
                cur_game.add_points(100 + time_bonus)
...
class SriScoreBoardView(arcade.View):
    ...
    def on_draw(self):
        ...
        for i in range(len(top_5_games)):
            score_text = (f'{i + 1}.' +
                          f'"{(top_5_games[i]).get_player()}"' +
                          ' ---- ' +
                          f'{(top_5_games[i]).get_points()} ' +
                          'points ---- ' +
                          f'{top_5_games[i].get_words_linked()}' +
                          ' words linked')
...
class SriEndGameView(arcade.View):
    ...
    def on_draw(self):
        ...
        arcade.draw_text((f"Player: {cur_game.get_player()}\n\n" +
                          f"Words Linked: " +
                          f"{cur_game.get_words_linked()}\n\n" +
                          f"Points: {cur_game.get_points()}\n\n" +
                          f"Rank: {cur_game.find_rank()}"),
...
class Game(Score):
    ...
    def __init__(self, current_player: str):
        ...
        The Game class inherits from the Score Class
    ...
    def calculate_points(self) -> None:
        ...
        """
        base_score = 10 * self.get_words_linked()
